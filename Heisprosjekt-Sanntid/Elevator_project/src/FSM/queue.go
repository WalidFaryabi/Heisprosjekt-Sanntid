...
func calculateOrderScore(floor int, button int)(float64){ // algorithm for calculating whether an elevator should take an order or not.
	...
	Println("We got at this point:  1")
	priorityScore-= float64(nCurrentOrders)*-10
...
if(lastFloor == nFloors-1){
	...
	Println("We survived this point : 2 ")
	if(button == BUTTON_CALL_UP){
		...
		if(lastDirection == 1){
			...
			priorityScore-= multiplierOfOrdersUp*10.0
		}
	}
	...
	Println("got here so far with priority score:")
	...
	if(lastFloor == floor){
		...
		Println("THIS SHOULD BE ACTIVATED")
		if(mainFloor < floor){
			priorityScore+=45
			if(button == BUTTON_CALL_DOWN){
				Println("this point should be given.")
				priorityScore+=155
			}
		}else if(mainFloor > floor && lastDirection == -1){
			priorityScore+=10
